...
async def redis_incident_consumer_and_rerouter():
    ...
    logger.info("[MAS02 Worker] 테스트를 위해 컨슈머 그룹 초기화를 시작합니다...")
    for stream_key in stream_keys:
        try:
            ...
            logger.info(f"[MAS02 Worker] 기존 그룹 삭제 완료: {stream_key}")
        except Exception:
            pass
    ...
    logger.info("[MAS02 worekrs.py redis_incident_consumer_and_rerouter] 서울시 25개 구 사고 스트림 동시 관제를 시작합니다... 무전 대기 중.")
...
async def patrol_active_incidents_loop(stream_keys, redis_client, group_name):
    ...
    """
    logger.info("[MAS02 순찰대] 중간 난입 유저 포착 스케줄러가 백그라운드에서 기동되었습니다.")
    ...
    while True:
        try:
            ...
            logger.info("[MAS02 순찰대] 백그라운드 태스크가 종료됩니다.")
            break
        except Exception as e:
            logger.error(f"[MAS02 순찰 루프 예외] 에러 발생 후 대기 처리합니다: {e}")
            await asyncio.sleep(5)